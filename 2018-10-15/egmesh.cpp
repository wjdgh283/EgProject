...
*
*	\param Verts 1링 이웃 정점들의 포인터가 저장된다.
*
...
*
*	\param Faces 정점을 공유하는 삼각형들의 포인터가 저장된다.
*
...
EgFace::EgFace(EgEdge *pEdge0, EgEdge *pEdge1, EgEdge *pEdge2, EgMaterial *pMtl)
{
	...
	// 에지의 연결관계를 생성한다.
	m_pEdge = pEdge0;
	pEdge0->m_pNext = pEdge1;
	pEdge1->m_pNext = pEdge2;
	pEdge2->m_pNext = pEdge0;
...
*
*	\param eIdx 찾을 에지의 인덱스(0 부터)
*
*	\return 해당 인덱스의 에지 포인터를 반환한다.
*/
...
/*!
*	\brief 삼각형 에지를 공유한 세 삼각형을 구한다.
*
*	\param Faces 에지를 공유한 세 삼각형의 포인터가 저장된다.
*/
void EgFace::GetIncidentFace(std::vector<EgFace *> &Faces)
{
	Faces.clear();
	EgEdge *e = m_pEdge;
	do
...
*	\param GroupName 삼각형이 포함된 그룹의 이름
*/
void EgMesh::AddFace(EgFace *pFace, std::string GroupName)
{
	ADD_TO_PTR_LIST(m_pFaces, pFace);
	if (!GroupName.empty())
		m_GroupedFaces[GroupName].push_back(pFace);
}
...
*
*	\return 메쉬의 남은 삼각형이 있다면 true, 없다면 false를 반환한다.
*/
...
*
*	\param Verts 제거할 정점의 포인터 배열
*
*	\return 메쉬의 남은 정점이 있다면 true, 없다면 false를 반환한다.
*/
...
*
*	\return 남은 삼각형이 있으면 true, 없으면 false를 반환한다.
*/
...
/*!
*	\brief 메쉬의 에지의 반대편 에지(Mate) 정보를 설정한다.
...
*	\brief 메쉬의 법선을 재계산한다.
*
*	\param bSmooth 법선의 형태(true: 평균 법선, false: 삼각형 법선)
*/
...
/*!
*	\brief 메쉬의 텍스처 경계를 렌더링 한다.
*/
void EgMesh::RenderTexBndry()
{
	if (m_pTexels == NULL)
		return;
	...
	while (pFace != NULL)
	{
		...
		if (e0->m_pMate != NULL)
		{
			if (e1->m_pTexel->m_Coord != e0->m_pMate->m_pTexel->m_Coord)
			{
				glVertex3dv(v0->m_Pos.V);
				glVertex3dv(v1->m_Pos.V);
			}
		...
		if (e1->m_pMate != NULL)
		{
			if (e2->m_pTexel->m_Coord != e1->m_pMate->m_pTexel->m_Coord)
			{
				glVertex3dv(v1->m_Pos.V);
				glVertex3dv(v2->m_Pos.V);
			}
		...
		if (e2->m_pMate != NULL)
		{
			if (e0->m_pTexel->m_Coord != e2->m_pMate->m_pTexel->m_Coord)
			{
				glVertex3dv(v2->m_Pos.V);
				glVertex3dv(v0->m_Pos.V);
			}
